...
plmap = {"PL-1": "*komo", "PL-2": "*jamo", "PL-3": "*tomo", "PL-4": "*ʔne", "PL-5": "*ʔno", "pempl": "ʔno"}
def get_pl(rec):
    out = []
    for pl, f in plmap.items():
        if pl in rec["Cognates"]:
            out.append(f)
    if len(out) == 0:
        out.append("none")
    return " + ".join(out)

t = forms[forms["Cognateset_ID"] == "2PL"]
t["Value"] = t.apply(lambda x: get_pl(x), axis=1)
plot_map(t, "2pl")

t = forms[forms["Cognateset_ID"] == "1+2PL"]
t["Value"] = t.apply(lambda x: get_pl(x), axis=1)
t = t[t["Value"] != "none"]
print(t)
plot_map(t, "12pl")
...
t = sort_by(t, "Value", ["*mə-initial", "*mə-loss"])
plot_map(t, "med-m")
